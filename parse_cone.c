...
static int parse_point(t_win *win, int fd, char *line)
{
    ...
    if (get_next_line(fd, &line) > 0)
    {
        ...
        if (curr[0])
            ((t_cone*)(win->objects[win->nb_obj - 1]))->point.x = ft_atof(curr[0]);
        else
            return (1);
        if (curr[1])
            ((t_cone*)(win->objects[win->nb_obj - 1]))->point.y = ft_atof(curr[1]);
        else
            return (1);
        if (curr[2])
...
static int parse_axis(t_win *win, int fd, char *line)
{
    ...
    if (get_next_line(fd, &line) > 0)
    {
        curr = ft_strsplit(line, ' ');
        if (curr[0])
            ((t_cone*)(win->objects[win->nb_obj - 1]))->axis.x = ft_atof(curr[0]);
        else
            return (1);
        if (curr[1])
            ((t_cone*)(win->objects[win->nb_obj - 1]))->axis.y = ft_atof(curr[1]);
        else
            return (1);
        if (curr[2])
            ((t_cone*)(win->objects[win->nb_obj - 1]))->axis.z = ft_atof(curr[2]);
        else
            return (1);
