...
/*   Updated: 2020/02/04 11:36:51 by cbilga           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_center(t_win *win, int fd, char *line)
{
	...
	if (get_next_line(fd, &line) > 0)
	{
		...
		{
			((t_sphere*)(win->objects[win->nb_obj - 1]))->center.x =
				ft_atof(curr[0]);
			((t_sphere*)(win->objects[win->nb_obj - 1]))->center.y =
				ft_atof(curr[1]);
			((t_sphere*)(win->objects[win->nb_obj - 1]))->center.z =
				ft_atof(curr[2]);
		}
		else
			return (1);
...
static int	parse_direction(t_win *win, int fd, char *line)
{
	...
	if (get_next_line(fd, &line) > 0)
	{
		...
		if (curr[0] && curr[1] && curr[2])
		{
			((t_sphere*)(win->objects[win->nb_obj - 1]))->dir.x =
				ft_atof(curr[0]);
			((t_sphere*)(win->objects[win->nb_obj - 1]))->dir.y =
				ft_atof(curr[1]);
			((t_sphere*)(win->objects[win->nb_obj - 1]))->dir.z =
				ft_atof(curr[2]);
		}
		else
			return (1);
